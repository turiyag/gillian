<?php
/*
Plugin Name: Twitter Widget @ KingSize for PHP 4
...
function widget_Twidget_init() {
	...
	function widget_Twidget_control() {
		...
		if ( !is_array($options) )
			$options = array('account'=>'', 'title'=>'Twitter Updates', 'show'=>'5');
		...
		echo '<p style="text-align:right;">
				<label for="Twitter-show">' . __('Set # of tweets to display:') . '
	...
	register_sidebar_widget(array('KingSize Twitter', 'widgets'), 'widget_Twidget');

	// Register settings for use, 300x200 pixel form
	register_widget_control(array('KingSize Twitter', 'widgets'), 'widget_Twidget_control', 300, 200);
}
